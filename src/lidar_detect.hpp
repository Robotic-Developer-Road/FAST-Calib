...
public:
    ...
    LidarDetect(std::shared_ptr<rclcpp::Node> &nh, Params &params)
          ...
          center_z0_cloud_(new pcl::PointCloud<pcl::PointXYZ>)
    {
        ...
        filtered_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("filtered_cloud", 1);
        plane_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("plane_cloud", 1);
        aligned_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("aligned_cloud", 1);
        edge_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("edge_cloud", 1);
        center_z0_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("center_z0_cloud", 10);
        center_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("center_cloud", 10);
    }

    void detect_solid_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        detect_lidar(cloud, center_cloud);
    }
    ...
    void detect_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        // 1. X、Y、Z方向滤波
        ...
        RCLCPP_INFO(rclcpp::get_logger("lidar_detect"), "Filtered cloud size: %ld", filtered_cloud_->size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("lidar_detect"), "Plane cloud size: %ld", plane_cloud_->size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("lidar_detect"), "Extracted %ld edge points.", edge_cloud_->size());
        ...
        RCLCPP_INFO(rclcpp::get_logger("lidar_detect"), "Number of edge clusters: %ld", cluster_indices.size());
